...
def stats(pix_location, pix_count, inFile):
    ...
    with  open(re.sub("$", "_stats.csv", inFile),"w" ,newline="") as statsFile:
        ...
        for c in pix_location.keys():
            writer.writerow([colorNames[c],len(pix_location[c]),(len(pix_location[c])/pix_count)*100])
        writer.writerow(["total", pix_count, 100])


    print("\nResult:")
    for c in pix_location.keys():
        print(colorNames[c]+": "+str(len(pix_location[c]))+"pixels (" + str((len(pix_location[c])/pix_count)*100) +"%)")
